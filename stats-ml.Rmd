...
plot(m)
```
The predict plot gives clear changes on variables, which can help us make easy predictions. For the residual plot, the Scale-Location shows many outliers plots, 
The linear regression fit is the most common prediction. The biggest advantage of this models is linearity. It makes the estimation procedure simple and these linear equations have an easy to understand. Sometimes it can be the 1st model to use. But the disadvantage is that it only Looks at the Mean of the Dependent Variable
...
Random Forest in R has an advantage, which is its avoids overfitting. (But my r is not 4.2.2, there is something wrong with the plotting and coding, sorry about that)
...
Poly code is not same as the linear model, it can help understand the true underlying relationship. From the plot, you can see the line is not that linear, which means ploy may be better use in the situation. 
Besides residuals, The sum of squares due to error (SSE) R-square Adjusted R-square Root mean squared error (RMSE) are ways to check the cubic fit. 
...
Check outcomes, residuals, r squares?
